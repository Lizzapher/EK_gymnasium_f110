...
# The file to continue training on, input file name
_NETWORK_ZIP_FILE = f"PPO_f110-10000000-batch_MultiTrack_Exp2-1_rep0"

# The output file name
_TEST_BATCH = f"_MultiTrack_{EXP}_{REPEAT}"
_LOG_NAME = f"10,000,000 total time steps - MultiTrack - {EXP} - {REPEAT}"

MAP_LIST = ['Catalunya', 'Monza', 'Shanghai', 'Spa', 'YasMarina', 'BrandsHatch', 'Montreal', 'IMS']
_TOTAL_TIME_STEPS = 10_000_000
_ADDITIONAL_TRAINING_STEPS = 10_000_000
...
def make_f1tenth_env_multi():
    ...
    env = gym.make(
        ...
        config=config,
        render_mode= None # for QtPy use 'human' for headless use None
    )
...
def main():
    ...
    print(f'\n[INFO] INPUT model name:  {_NETWORK_ZIP_FILE}.zip, entropy: {_ENT_COEF}')
    print(f'\n[INFO] OUPUT model name: PPO_f110-{_NEW_TOTAL_STEPS}-batch{_TEST_BATCH}, \nmap: {MAP_LIST}, \nentropy: {_ENT_COEF}, \nprevious training: {_TOTAL_TIME_STEPS}, \nadditional training: {_ADDITIONAL_TRAINING_STEPS}, \nnew total time steps: {_NEW_TOTAL_STEPS}\n')
    ...
    model.save(f"./models/PPO_-{_NEW_TOTAL_STEPS}_batch{_TEST_BATCH}")
